...
const CATEGORY_COLORS = {
  ...
  Backend: "bg-rose-100 text-rose-700 border-rose-200",
  DevOps: "bg-fuchsia-100 text-fuchsia-700 border-fuchsia-200",
  AI: "bg-purple-100 text-purple-700 border-purple-200",
  Mobile: "bg-pink-100 text-pink-800 border-pink-300",
  "UI/UX": "bg-rose-100 text-rose-800 border-rose-300",
};
...
const ResourceCard = ({
  ...
  return (
    <div className="group relative flex flex-col justify-between rounded-3xl border border-pink-200/90 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-pink-300 hover:shadow-xl hover:shadow-pink-100/60">
      {/* Top Meta Bar */}
      <div>
        ...
        <h3 className="mt-4 text-lg font-bold text-slate-900 transition-colors group-hover:text-pink-600">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex items-start justify-between gap-2 text-slate-900 hover:text-pink-700"
          >
        ...
        {resource.tags && resource.tags.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-1.5">
            {resource.tags.map((tag, index) => (
              <span
                key={`${tag}-${index}`}
                className="inline-flex items-center gap-1 rounded-lg bg-pink-50 px-2.5 py-0.5 text-[11px] font-medium text-pink-700 hover:bg-pink-100"
              >
      ...
      <div className="mt-6 flex items-center justify-between border-t border-pink-100 pt-4">
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-xl border border-pink-200 bg-pink-50/60 px-3.5 py-1.5 text-xs font-bold text-pink-700 shadow-sm transition-all hover:border-pink-300 hover:bg-pink-100 hover:text-pink-800 active:scale-95 cursor-pointer"
        >
